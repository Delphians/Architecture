...
fun DrawChart(
    ...
    Canvas(
        ...
        val cubicPointsFirst = mutableListOf<PointF>()
        val cubicPointsSecond = mutableListOf<PointF>()

        for (i in 1 until points.size) {
            cubicPointsFirst.add(PointF((points[i].x + points[i - 1].x) / 2, points[i - 1].y))
            cubicPointsSecond.add(PointF((points[i].x + points[i - 1].x) / 2, points[i].y))
        }
