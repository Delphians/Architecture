...
* it's necessary to have Tiingo token (sign-up for a free account) to get the real data from Tiingo
  (https://www.tiingo.com/) and put the token into `buildConfigField` in `BaseExtensionConfigs`
  file. Also, in `GetTickerPriceIntentionProcessor` (`features:payments` module) use 
  `dailyTickerPricesUseCase` (this line is commented out).
